...
@section('title', 'About Edit') <!--title-->
@section('nav_about', 'active') <!--讓nav_about=avtive 用來觸發narbar裡的選單標籤活動狀態-->
@section('content')<!-- 以下是內容-->

<div class="container">

    <section class="page-section my-5 p-5"><!-- 引用自訂class 外距3倍rem(48px) 內距3倍rem(48px)-->
        <!--使用POST方式傳遞 傳遞到about.upsdate enctype=傳送前不對字符進行編碼-->
        <form method="POST" action="{{ route('admin.about.update') }}" enctype="multipart/form-data">

            {{ csrf_field() }}<!-- 防止CSRF攻擊-->

            <div class="form-group"><!--表單-->
                <label for="content">關於我</label><!--輸入框標題 for=與哪個intput綁定-->
                <!--tinymce的文字編輯器 將輸入內容新增至about這個資料表裡的content欄位 並確認該輸入框是否有內容-->
